...
import {CreateUserInterface, DeleteUserInterface, ForgotPasswordInterface, GetSingleUserInterface, ResetPasswordInterface, UpdateUserDetailsInterface, UpdateUserPasswordInterface, VerifyUserInterface} from '@/resources/user/user.interface';
...
class UserController implements Controller {
    ...
    private verify = async (req: Request<{}, {}, {}, VerifyUserInterface>, res: Response, next: NextFunction): Promise<Response | void> => {
    ...
    private forgotPassword = async (req: Request<{}, {}, ForgotPasswordInterface>, res: Response, next: NextFunction): Promise<Response | void> => {
        ...
        try {
            const message = await this.UserService.forgotPassword(forgotPasswordInput);
            res.status(StatusCodes.OK).send(message);
        } catch (e: any) {
            log.error(e.message);
    ...
    private resetPassword = async (req: Request<{}, {}, ResetPasswordInterface['body'], ResetPasswordInterface['query']>, res: Response, next: NextFunction): Promise<Response | void> => {
        const resetPasswordQuery = req.query;
        const resetPasswordBody = req.body;

        try {
            const message = await this.UserService.resetPassword(resetPasswordQuery, resetPasswordBody);
            res.status(StatusCodes.OK).send(message)
        } catch (e: any) {
            log.error(e.message);
            next(new HttpException(StatusCodes.BAD_REQUEST, e.message))
    ...
    private getCurrentUser = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
        ...
        if(user){
            ...
            next(new HttpException(StatusCodes.BAD_REQUEST, 'You are currently logged out'))
        }
    }

    private getAllUser = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    ...
    private getSingleUser = async (req: Request<GetSingleUserInterface>, res: Response, next: NextFunction): Promise<Response | void> => {
        const {role} = res.locals.user;
    ...
    private updateUserDetails = async (req: Request<{}, {}, UpdateUserDetailsInterface>, res: Response, next: NextFunction): Promise<Response | void> => {
        const {_id: userId} = res.locals.user;
        const updateData = req.body;

        try {
            const message = await this.UserService.updateUserDetails(updateData, userId);
            res.status(StatusCodes.OK).send(message)
        } catch (e: any) {
            log.error(e.message);
            next(new HttpException(StatusCodes.BAD_REQUEST, e.message))
    ...
    private updateUserPassword = async(req: Request<{}, {}, UpdateUserPasswordInterface>, res:Response, next: NextFunction): Promise<Response | void> => {
        const updatePasswordInput = req.body;
        const {_id: userId} = res.locals.user;
        try {
            const message = await this.UserService.updateUserPassword(updatePasswordInput, userId);
            res.status(StatusCodes.OK).send(message);
        } catch (e: any) {
            log.error(e.message);
            next(new HttpException(StatusCodes.BAD_REQUEST, e.message));
    ...
    private deleteUser = async(req: Request<{}, {}, DeleteUserInterface>, res: Response, next: NextFunction): Promise<Response | void> => {
        const deleteUserInput = req.body;
        const {_id: userId} = res.locals.user;
        try {
            const message = await this.UserService.deleteUser(deleteUserInput, userId);
            res.status(StatusCodes.OK).send(message);
        } catch (e: any) {
            log.error(e.message);
            next(new HttpException(StatusCodes.FORBIDDEN, e.message));
